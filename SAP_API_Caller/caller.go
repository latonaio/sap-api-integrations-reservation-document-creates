...
import (
	...
	sap_api_output_formatter "sap-api-integrations-reservation-document-creates/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Header(header *requests.Header) {
	outputDataHeader, err := c.callReservationDocumentSrvAPIRequirementHeader("A_ReservationDocumentHeader", header)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callReservationDocumentSrvAPIRequirementHeader(api string, header *requests.Header) (*sap_api_output_formatter.Header, error) {
...
func (c *SAPAPICaller) Item(item *requests.Item) {
	outputDataItem, err := c.callReservationDocumentSrvAPIRequirementItem("A_ReservationDocumentItem", item)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callReservationDocumentSrvAPIRequirementItem(api string, item *requests.Item) (*sap_api_output_formatter.Item, error) {
